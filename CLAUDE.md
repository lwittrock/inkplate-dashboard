...
| `FULL_REFRESH_EVERY` | Meant as wakes between full e-ink refreshes (default 4). Has no effect: every refresh is full, see the 1-bit display note |
...
- **1-bit display**: All drawing is BLACK or WHITE only. `display.display()` causes a full e-ink refresh (~1–2 s, some flicker). **`partialUpdate()` after deep sleep is a full refresh too** (found 23 September 2026): the library sets `_blockPartial` on every boot and only clears it after a full refresh, so every wake refreshes fully and `FULL_REFRESH_EVERY` has no effect. A real partial refresh after deep sleep needs `partialUpdate(true)` and the previous frame loaded into the library's buffer first; see docs/server-rendering-design.md, "Later".
