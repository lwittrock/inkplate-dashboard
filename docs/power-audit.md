...
> **CORRECTION — 23 September 2026: partial refreshes never happened.** The Inkplate library turns `partialUpdate()` into a full refresh on the first update after every boot (`_blockPartial` in `Inkplate6Driver.cpp`, library 11.1.0), and every wake is a boot after deep sleep. So every wake has been a full refresh: §3 row 12 and §4's e-ink bucket (modelled as ¼ full, ¾ partial) are too low, and the `FULL_REFRESH_EVERY` knob (§9 #6, rejected 2026-07-26) never had an effect. The panel has also never ghosted. The e-ink bucket was ~1.5% of the old budget, so the old totals barely move; for the server-rendered design it matters more (see `server-rendering-design.md`, "Later").
>
